...
class BanubaPlatformCameraView: NSObject, FlutterPlatformView {
    ...
    private func listenFlutterCalls(viewId: Int64, messenger: FlutterBinaryMessenger?) {
        ...
        channel?.setMethodCallHandler { [weak self] methodCall, resultHandler in
            ...
            case "startVideoRecording": self.handleStartVideoRecording(methodCall, result: resultHandler)
            case "stopVideoRecording": self.handleStopVideoRecording(methodCall, result: resultHandler)
    ...
    private func handleStartVideoRecording(_ methodCall: FlutterMethodCall, result: FlutterResult) {
        let dictionary = methodCall.arguments as? Dictionary<String, Any>
        let filePath = dictionary!["filePath"] as? String ?? ""
        let recordAudio = dictionary!["recordAudio"] as? Bool ?? false
    ...
    private func handleStopVideoRecording(_ methodCall: FlutterMethodCall, result: FlutterResult) {
